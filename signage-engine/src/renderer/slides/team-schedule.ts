...
export class TeamScheduleSlide extends BaseSlide {
  render(ctx: SKRSContext2D, data: DataCache, _deltaTime: number): void {
    ...
    schedule.forEach((user, userIndex) => {
      ...
      drawText(
        ctx,
        truncateText(ctx, user.userName, nameColWidth - 20, this.displayConfig.fontFamily, 44),
        padding + 10,
        rowY + rowHeight / 2,
        {
          font: this.displayConfig.fontFamily,
      ...
      user.assignments.forEach((assignment) => {
        ...
        if (dayIndex >= 0) {
          ...
          if (blockWidth > 100) {
            drawText(
              ctx,
              truncateText(ctx, assignment.projectName, blockWidth - 20, this.displayConfig.fontFamily, this.FONT_SIZE.MINIMUM),
              blockX + blockWidth / 2,
              blockY + blockHeight / 2 - 24,
              {
                font: this.displayConfig.fontFamily,
