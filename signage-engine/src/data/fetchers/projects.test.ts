...
describe('fetchActiveProjects', () => {
  ...
  describe('mock data fallback', () => {
    ...
    it('should log debug message about mock data', async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        'Supabase not configured, returning mock projects'
      );
    });
  ...
  describe('ActiveProject interface compliance', () => {
    ...
    it('should have nullable fields as string or null', async () => {
      ...
      projects.forEach((project) => {
        expect(
          project.project_type === null || typeof project.project_type === 'string'
        ).toBe(true);
        expect(
          project.salesperson === null || typeof project.salesperson === 'string'
        ).toBe(true);
        expect(
          project.start_date === null || typeof project.start_date === 'string'
        ).toBe(true);
        expect(
          project.due_date === null || typeof project.due_date === 'string'
        ).toBe(true);
      });
    });
  });
