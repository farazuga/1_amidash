...
export async function fetchProjectMetrics(): Promise<ProjectMetrics> {
  ...
  try {
    ...
    for (const project of projects || []) {
      const status = project.statuses as unknown as { id: string; name: string } | null;
      const projectType = project.project_types as unknown as { name: string } | null;
    ...
    const completedThisWeek = (recentCompletions || []).filter(
      c => c.changed_at && new Date(c.changed_at) >= weekStart
    ).length;
    ...
    const byType: TypeCount[] = Array.from(typeCounts.entries())
...
function getMockMetrics(): ProjectMetrics {
  return {
    total: 24,
    byStatus: [
