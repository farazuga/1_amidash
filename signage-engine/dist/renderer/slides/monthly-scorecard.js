...
export class MonthlyScorecardSlide extends BaseSlide {
    ...
    drawEmptyState(ctx, headerHeight) {
    ...
    drawRevenueSection(ctx, revenue, x, y, width) {
    ...
    drawProjectMetricsSection(ctx, metrics, x, y, width) {
        ...
        if (metrics) {
            ...
            drawKPICard(ctx, 'Completed', metrics.completedThisMonth.toString(), 'This Month', x + innerPadding, innerY, cardWidth, cardHeight, {
                backgroundColor: hexToRgba(colors.success, 0.2),
                valueColor: colors.success,
            });
            // Active Projects
            drawKPICard(ctx, 'Active', metrics.total.toString(), 'Total Projects', x + innerPadding + cardWidth + gap, innerY, cardWidth, cardHeight, {
                backgroundColor: hexToRgba(colors.info, 0.2),
                valueColor: colors.info,
            });
            // Upcoming Deadlines
            drawKPICard(ctx, 'Due Soon', metrics.upcomingDeadlines.toString(), 'Next 7 Days', x + innerPadding, innerY + cardHeight + gap, cardWidth, cardHeight, {
                backgroundColor: hexToRgba(colors.warning, 0.2),
                valueColor: colors.warning,
            });
            // Overdue
            drawKPICard(ctx, 'Overdue', metrics.overdueCount.toString(), 'Needs Attention', x + innerPadding + cardWidth + gap, innerY + cardHeight + gap, cardWidth, cardHeight, {
    ...
    drawLargeMetricCard(ctx, title, value, subtitle, progress, isOnTrack, x, y, width, height) {
        ...
        drawProgressBar(ctx, Math.min(progress, 100), 100, x + padding, progressY, width - padding * 2, 24, {
