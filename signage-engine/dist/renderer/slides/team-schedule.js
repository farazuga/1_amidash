...
export class TeamScheduleSlide extends BaseSlide {
    render(ctx, data, _deltaTime) {
        ...
        schedule.forEach((user, userIndex) => {
            ...
            // User name
            drawText(ctx, truncateText(ctx, user.userName, nameColWidth - 20, this.displayConfig.fontFamily, 32), padding + 10, rowY + rowHeight / 2, {
                font: this.displayConfig.fontFamily,
                size: 32,
                color: colors.white,
                baseline: 'middle',
            ...
            user.assignments.forEach((assignment) => {
                ...
                if (dayIndex >= 0 && dayIndex < daysToShow) {
                    ...
                    if (blockWidth > 60) {
                        drawText(ctx, truncateText(ctx, assignment.projectName, blockWidth - 8, this.displayConfig.fontFamily, 18), blockX + blockWidth / 2, blockY + blockHeight / 2 - 12, {
                            font: this.displayConfig.fontFamily,
                            size: 18,
                            color: colors.white,
                            align: 'center',
