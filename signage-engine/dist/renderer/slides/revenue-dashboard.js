...
export class RevenueDashboardSlide extends BaseSlide {
    render(ctx, data, _deltaTime) {
        ...
        drawKPICard(ctx, 'This Month', `$${(revenue.currentMonthRevenue / 1000).toFixed(0)}K`, `Goal: $${(revenue.currentMonthGoal / 1000).toFixed(0)}K`, padding, contentY, cardWidth, cardHeight);
        // Month Progress
        const monthProgress = revenue.currentMonthGoal > 0
            ? (revenue.currentMonthRevenue / revenue.currentMonthGoal * 100).toFixed(0)
            : '0';
        drawKPICard(ctx, 'Month Progress', `${monthProgress}%`, revenue.currentMonthRevenue >= revenue.currentMonthGoal ? 'On Track' : 'Behind', padding + cardWidth + cardGap, contentY, cardWidth, cardHeight, {
            valueColor: revenue.currentMonthRevenue >= revenue.currentMonthGoal ? colors.success : colors.warning,
        });
        // YTD Revenue
        drawKPICard(ctx, 'Year to Date', `$${(revenue.yearToDateRevenue / 1000000).toFixed(2)}M`, `Goal: $${(revenue.yearToDateGoal / 1000000).toFixed(2)}M`, padding + (cardWidth + cardGap) * 2, contentY, cardWidth, cardHeight);
        // YTD Progress
