 ...
 */
export function drawGauge(ctx, value, centerX, centerY, radius, options = {}) {
    const { title, subtitle, valueLabel, minValue = 0, maxValue = 100, thresholds = { low: 60, medium: 80 }, showNeedle = true, animated = false, animationProgress = 1, } = options;
 ...
 */
export function getGaugeColor(percentage, thresholds) {
 ...
 */
export function drawMiniGauge(ctx, value, centerX, centerY, radius, label, thresholds = { low: 60, medium: 80 }) {
 ...
 */
export function drawHorizontalGauge(ctx, value, x, y, width, height, options = {}) {
    const { label, showValue = true, thresholds = { low: 60, medium: 80 }, } = options;
    const normalizedValue = Math.max(0, Math.min(100, value));
    const fillColor = getGaugeColor(normalizedValue, thresholds);
