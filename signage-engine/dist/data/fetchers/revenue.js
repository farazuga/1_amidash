...
export async function fetchRevenueData() {
    ...
    try {
      ...
      `)
        ...
        (invoiced || []).forEach((item) => {
            const date = new Date(item.changed_at);
            const month = date.getMonth() + 1;
            const value = item.projects?.total_value || 0;
        ...
        return {
            ...
            currentMonthGoal: goals?.find((g) => g.month === currentMonth)?.amount || 0,
...
async function getInvoicedStatusId() {
    if (!supabase)
        return null;
