...
export async function fetchDashboardMetrics() {
    ...
    try {
        ...
        const [projectsResult, statusesResult, statusHistoryResult, goalsResult,] = await Promise.all([
            supabase
                .from('projects')
                .select(`
          id,
        ...
        const invoicedStatus = statuses.find((s) => s.name.toLowerCase().includes('invoiced'));
        const invoicedStatusId = invoicedStatus?.id;
        // Filter active projects (not invoiced)
        const activeProjects = projects.filter((p) => p.current_status_id !== invoicedStatusId);
        // Calculate all metrics
        const health = calculateHealthMetrics(projects, statusHistory, goals, statuses, currentMonth, invoicedStatusId);
        const alerts = calculateAlertsData(activeProjects, statusHistory, invoicedStatusId);
        const performance = calculatePerformanceMetrics(projects, activeProjects, statusHistory, invoicedStatusId);
        const velocity = calculateVelocityData(projects, statusHistory, invoicedStatusId);
        const cycleTime = calculateCycleTimeData(projects, statusHistory, statuses, invoicedStatusId);
...
// ==========================================
function calculateHealthMetrics(projects, statusHistory, goals, statuses, currentMonth, invoicedStatusId) {
    const now = new Date();
    const monthStart = startOfMonth(now);
    const monthEnd = endOfMonth(now);
    // Get current month goal
    const monthGoal = goals.find((g) => g.month === currentMonth);
    const goalAmount = monthGoal?.revenue_goal || 0;
    ...
    const posRevenue = posThisMonth.reduce((sum, p) => sum + (p.sales_amount || 0), 0);
    // Calculate invoiced this month
    const invoicedThisMonth = statusHistory.filter((h) => {
        const status = h.statuses;
        if (status?.name?.toLowerCase().includes('invoiced') === false)
            return false;
    ...
    const procurementStatus = statuses.find((s) => s.name.toLowerCase().includes('procurement') ||
        s.name.toLowerCase().includes('material'));
    const engineeringStatus = statuses.find((s) => s.name.toLowerCase().includes('engineering') ||
        s.name.toLowerCase().includes('design'));
    const procurementCount = projects.filter((p) => p.current_status_id === procurementStatus?.id).length;
    const engineeringCount = projects.filter((p) => p.current_status_id === engineeringStatus?.id).length;
    // Determine diagnosis
    let diagnosis;
    let message;
...
function calculateAlertsData(activeProjects, statusHistory, invoicedStatusId) {
    const now = new Date();
    // Find stuck projects (in same status > threshold days)
    const stuckProjects = [];
    const projectLastStatusChange = new Map();
    ...
    activeProjects.forEach((p) => {
        const lastChange = projectLastStatusChange.get(p.id);
        if (!lastChange)
            return;
    ...
    const overdueProjects = activeProjects
        ...
        const goalDate = p.goal_completion_date
            ? new Date(p.goal_completion_date)
            : null;
    ...
    return {
        stuckProjects: stuckProjects.slice(0, 10),
        overdueProjects: overdueProjects.slice(0, 10),
...
function calculatePerformanceMetrics(projects, activeProjects, statusHistory, invoicedStatusId) {
    ...
    invoicedHistory.forEach((h) => {
        ...
        if (!project)
            return;
        const invoicedAt = new Date(h.changed_at);
    ...
    const onTimePercent = onTime + late > 0 ? (onTime / (onTime + late)) * 100 : 0;
    ...
    const pipelineRevenue = activeProjects.reduce((sum, p) => sum + (p.sales_amount || 0), 0);
    const backlogDepth = avgMonthlyInvoiced > 0
    ...
    activeProjects.forEach((p) => {
        ...
        clientRevenue.set(client, (clientRevenue.get(client) || 0) + (p.sales_amount || 0));
    });
    ...
    const customerConcentration = totalRevenue > 0 ? (top3Revenue / totalRevenue) * 100 : 0;
...
function calculateVelocityData(projects, statusHistory, invoicedStatusId) {
    const now = new Date();
    const monthly = [];
    for (let i = 5; i >= 0; i--) {
        ...
        const invoicedInMonth = statusHistory.filter((h) => {
            const status = h.statuses;
            if (!status?.name?.toLowerCase().includes('invoiced'))
                return false;
            const changedAt = new Date(h.changed_at);
...
function calculateCycleTimeData(projects, statusHistory, statuses, invoicedStatusId) {
    ...
    projectHistories.forEach((history, projectId) => {
        ...
        history.sort((a, b) => new Date(a.changed_at).getTime() -
        ...
        for (let i = 0; i < history.length; i++) {
            ...
            if (!statusName || statusName.toLowerCase().includes('invoiced'))
                continue;
    ...
    // Calculate averages
    const cycleTimeStatuses = Array.from(statusTimes.entries())
        .map(([name, data]) => {
        const isBottleneck = name.toLowerCase().includes('procurement') ||
    ...
    const totalAvgCycleTime = cycleTimeStatuses.reduce((sum, s) => sum + s.avgDays, 0);
    return {
        statuses: cycleTimeStatuses,
        totalAvgCycleTime,
    };
}
function calculateStatusPipeline(activeProjects, statuses) {
    ...
    activeProjects.forEach((p) => {
        ...
        if (existing) {
            existing.count++;
            existing.revenue += p.sales_amount || 0;
        }
        else {
            statusCounts.set(statusName, {
                count: 1,
                revenue: p.sales_amount || 0,
                displayOrder: statusInfo?.display_order || 999,
            });
        }
    });
    const pipelineStatuses = Array.from(statusCounts.entries())
        .map(([name, data]) => {
        const isBottleneck = name.toLowerCase().includes('procurement') ||
            name.toLowerCase().includes('engineering') ||
            name.toLowerCase().includes('material');
    ...
    return {
        ...
        totalRevenue: activeProjects.reduce((sum, p) => sum + (p.sales_amount || 0), 0),
    };
}
// ==========================================
// MOCK DATA
// ==========================================
function getMockDashboardMetrics() {
    return {
        health: {
            salesHealth: 85,
            opsHealth: 72,
