 ...
 */
// Project builders
export function buildActiveProject(overrides = {}) {
    return {
        id: '1',
        name: 'Test Project',
...
export function buildActiveProjects(count = 3) {
    return Array.from({ length: count }, (_, i) => buildActiveProject({
        id: `${i + 1}`,
        name: `Project ${i + 1}`,
...
// PO builders
export function buildRecentPO(overrides = {}) {
    return {
        id: '1',
...
export function buildRecentPOs(count = 3) {
    return Array.from({ length: count }, (_, i) => buildRecentPO({
        id: `${i + 1}`,
...
// Revenue builders
export function buildRevenueData(overrides = {}) {
    return {
...
// Schedule builders
export function buildScheduleEntry(overrides = {}) {
    return {
        userId: '1',
...
export function buildScheduleEntries(count = 3) {
    const names = ['Alice Johnson', 'Bob Smith', 'Carol Williams', 'David Brown', 'Eve Davis'];
    return Array.from({ length: count }, (_, i) => buildScheduleEntry({
        userId: `${i + 1}`,
        userName: names[i % names.length],
    }));
}
// Metrics builders
export function buildProjectMetrics(overrides = {}) {
    return {
        total: 50,
        active: 20,
        completed: 25,
...
export function buildSignageSlide(overrides = {}) {
    return {
        id: '1',
...
export function buildSignageSlides(count = 4) {
    const types = ['active-projects', 'revenue-dashboard', 'po-ticker', 'team-schedule'];
    return Array.from({ length: count }, (_, i) => buildSignageSlide({
        id: `${i + 1}`,
...
export function buildSupabaseResponse(data, error = null) {
    return { data, error };
}
export function buildSupabaseError(message, code = 'PGRST000') {
    return {
        data: null,
