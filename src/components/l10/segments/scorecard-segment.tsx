...
// Last 4 completed weeks (most recent first)
function getLast4CompletedWeeks(): string[] {
  const weeks: string[] = [];
  const now = new Date();
...
export function ScorecardSegment({ teamId }: ScorecardSegmentProps) {
  ...
  const weeks = getLast4CompletedWeeks();
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                ...
                {weeks.map((week) => (
                  <th key={week} className="px-2 py-2 text-center font-medium min-w-[80px] text-xs whitespace-nowrap">{formatWeekRange(week)}</th>
                ))}
            ...
            <tbody>
              {measurables.map((m) => {
                ...
                // Most recent completed week is first in the array
