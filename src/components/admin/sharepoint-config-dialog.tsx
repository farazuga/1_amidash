...
import { saveSharePointConfig } from '@/app/(dashboard)/admin/settings/sharepoint-actions';
...
interface SharePointConfigDialogProps {
  ...
  onConfigured: (config: SharePointGlobalConfig) => void;
}
...
export function SharePointConfigDialog({
  open,
  onOpenChange,
  onConfigured,
}: SharePointConfigDialogProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await saveSharePointConfig({
        siteId: selectedSite.id,
        siteName: selectedSite.displayName || selectedSite.name,
        driveId: selectedDrive.id,
        driveName: selectedDrive.name,
        baseFolderId: currentFolder.id,
        baseFolderPath: folderPathString || '/',
        baseFolderUrl: currentFolder.webUrl,
      });
    ...
    >
      <DialogContent className="max-w-lg max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 'sites' && 'Select the SharePoint site for all project files'}
            {step === 'drives' && 'Select a document library'}
            {step === 'folders' && 'Select the base folder for all projects'}
          ...
          {step === 'folders' && (
            <div className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  {folders.length === 0 && (
                    <p className="text-sm text-gray-500 py-4 text-center">
                      No subfolders. You can use this folder as the base.
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="pt-3 border-t text-sm text-gray-500">
                Select the folder where project subfolders will be created (e.g., &quot;/Projects&quot;)
              </div>
            </div>
          )}
          ...
          {step === 'confirm' && (
            <div className="space-y-4">
              <div className="p-4 rounded-lg bg-gray-50 space-y-3">
                ...
                <div>
                  <Label className="text-gray-500">Base Folder</Label>
              ...
              <div className="p-4 rounded-lg bg-blue-50 border border-blue-200 text-blue-800 text-sm">
                ...
                <ul className="list-disc list-inside space-y-1">
                  <li>All projects will store files in this location</li>
                  <li>Each project gets its own subfolder (e.g., /Projects/ClientName)</li>
                  <li>Category folders are created automatically (Schematics, SOW, etc.)</li>
                  <li>Users can upload files and they sync to SharePoint</li>
                </ul>
              </div>
            </div>
          )}
